...
export async function getGlobalNotionData({
  ...
  // 获取Notion数据；若失败则返回带默认空集合的稳定结构，避免下游 .map/.filter 崩溃。
  // raw 在彻底失败时上游会返回 []（数组），无脑 deepClone 会得到数组，Next.js 序列化 props 时会报错。
  const raw = await getNotionPageData({ pageId, from })
  const isUsableObject = raw && typeof raw === 'object' && !Array.isArray(raw)
  const notionPageData = isUsableObject ? (deepClone(raw) || {}) : {}
  notionPageData.allPages = notionPageData.allPages || []
  notionPageData.customNav = notionPageData.customNav || []
  notionPageData.categories = notionPageData.categories || []
  notionPageData.tags = notionPageData.tags || []
  notionPageData.tagOptions = notionPageData.tagOptions || []
  notionPageData.categoryOptions = notionPageData.categoryOptions || []
  notionPageData.latestPosts = notionPageData.latestPosts || []
  notionPageData.siteInfo = notionPageData.siteInfo || {
    title: BLOG.TITLE,
...
async function getPageRecordMapByNotionAPI({ pageId, from }) {
  ...
  // 当根页不是 collection_view_page（例如是一个普通 page，里面内嵌数据库）时，
  // 从其下的 collection_view 子块里推导 collectionId 与 viewIds，
  // 避免直接因 rawMetadata.type 不是数据库而返回 null。
  const inlineView = Object.values(collectionView || {})
    .map(v => v?.value)
    .find(v => v && (v.parent_id === pageId || v.collection_pointer))
  const collectionId = rawMetadata?.collection_id || inlineView?.collection_pointer?.id || inlineView?.collection_id
  const viewIds = rawMetadata?.view_ids || (inlineView?.id ? [inlineView.id] : null)
  if (!collection || !collectionId || !viewIds) {
    console.warn(`pageId "${pageId}" has no usable collection — please check your Notion template`)
    return null
  }
